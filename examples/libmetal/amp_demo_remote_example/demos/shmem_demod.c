 ...
 *
 * @param[in] ch->shm_io - metal i/o region of the shared memory
...
int shmem_demod(struct channel_s *ch)
{
	...
	while (metal_io_read32(ch->shm_io, SHM_DEMO_CNTRL_OFFSET) !=
		DEMO_STATUS_START);
	...
	while(metal_io_read32(ch->shm_io, SHM_DEMO_CNTRL_OFFSET) ==
		DEMO_STATUS_START) {
		if (metal_io_read32(ch->shm_io, SHM_RX_AVAIL_OFFSET)
			== rx_count)
			continue;
		/* Message is available, read the message header */
		ret = metal_io_block_read(ch->shm_io, SHM_RX_BUFFER_OFFSET,
		...
		ret = metal_io_block_read(ch->shm_io,
		...
		metal_io_write32(ch->shm_io, SHM_TX_AVAIL_OFFSET, rx_count);
	}
